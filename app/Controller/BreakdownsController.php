...
class BreakdownsController extends AppController{
    ...
    # ブランド設定
    private function init_brand($location){
        $brand_name = "";
        if($location['Location']['name']=='池袋店'){
            $brand_name = array("寿し和");
        }
        elseif($location['Location']['name']=='赤羽店'){
            $brand_name = array("寿し和");
        }
        elseif($location['Location']['name']=='和光店'){
            $brand_name = array("寿し和", "和香苑");
        }
        return $brand_name;
    }

    # m_breakdowns検索
    private function init_breakdowns($brand_name){
        $arr = [];
        # 第一階層内訳
        $m_breakdowns = $this->MBreakdown->find('all', array(
            'conditions' => array('MBreakdown.name' => $brand_name)
        ));
        if($m_breakdowns!=null){
            foreach($m_breakdowns as $m_breakdown){
                # 第二階層内訳
                $m_breakdowns1 = $this->MBreakdown->find('all', array(
                    'conditions' => array('MBreakdown.parent_id' => $m_breakdown['MBreakdown']['id'])
                ));
                if($m_breakdowns1!=null){
                    foreach($m_breakdowns1 as $m_breakdown1){
                        # 第三階層内訳
                        $m_breakdowns2 = $this->MBreakdown->find('all', array(
                            'conditions' => array('MBreakdown.parent_id' => $m_breakdown1['MBreakdown']['id'])
                        ));
                        if($m_breakdowns2!=null){
                            foreach($m_breakdowns2 as $m_breakdown2){
                                $m_breakdown1['Children'][] = $m_breakdown2;
                            }
                        }
                        $m_breakdown['Children'][] = $m_breakdown1;
                    }
                }
                $arr[] = $m_breakdown;
            }
